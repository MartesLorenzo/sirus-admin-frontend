...
export default function Clients() {
  const { clients, setClients, bookings, projects } = useAdmin();
  ...
  return <>
    <PageIntro eyebrow="CRM / RELAÇÕES" title="Clientes" description="Identificação, contactos, origem e histórico de reuniões. Os projetos são geridos na aba Projetos." action={<button className="button primary" onClick={() => open()}><Plus size={16} /> Novo cliente</button>} />
    ...
    <Panel title="Base de clientes" action={<Badge>{filtered.length} registos</Badge>}><div className="table-toolbar"><div className="searchbox"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nome, código, email ou empresa…" /></div><select value={stage} onChange={(event) => setStage(event.target.value)}>{stages.map((item) => <option key={item}>{item}</option>)}</select></div>
      <div className="table-wrap"><table><thead><tr><th>CLIENTE</th><th>EMPRESA / ORIGEM</th><th>REUNIÃO</th><th>PROJETOS</th><th>ESTADO</th><th /></tr></thead><tbody>{filtered.map((client) => <tr key={client.id}><td><b>{client.name}</b><small>{client.email} · {client.phone}</small></td><td><b>{client.company || "Particular"}</b><small>{client.origin || "Origem por definir"}</small></td><td><code>{client.bookingCode || "—"}</code></td><td>{projects.filter((project) => project.clientId === client.id).length}</td><td><Badge tone={client.status === "Finalizado" ? "success" : client.status === "Por responder" ? "warning" : ""}>{client.status}</Badge></td><td><button className="table-action" onClick={() => open(client)}>Editar →</button></td></tr>)}</tbody></table>{!filtered.length && <Empty>Nenhum cliente corresponde aos filtros.</Empty>}</div>
    </Panel>
    ...
    {editing.id && <button type="button" className="button subtle" onClick={resetPassword}>Gerar nova senha do cliente</button>}
